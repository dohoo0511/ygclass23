...
(async () => {
    ...
    const good = { users: { admin: {}, '1': { pi: 6 } }, note: '새 내용', lastSave: { build: 14 } };
    ...
    r = await call('PUT', '', { baseRev: 10, record: { users: { admin: {} }, note: '오래된 화면', lastSave: { build: 14 } } });
    ...
    r = await call('PUT', '', { baseRev: 11, record: { note: '학생 정보 없음', lastSave: { build: 14 } } });
    ...
    r = await call('PUT', '', { baseRev: 200, record: withBuild(14) });
    check('현재 빌드(14)의 저장은 통과', r.status === 200 && storage.users['1'].pi === 2);
    ...
    r = await call('PUT', '', { baseRev: 100, record: { users: { admin: { password: 'pw!' }, '1': { pi: 11 } }, note: '첫 저장', lastSave: { build: 14 } } });
    ...
    r = await call('PUT', '', { baseRev: 101, record: { users: { admin: { password: 'pw!' }, '1': { pi: 12 } }, lastSave: { build: 14 } } });
    ...
    r = await call('PUT', '', { baseRev: 900, record: { users: { admin: {} }, note: 'KV 없음', lastSave: { build: 14 } } });
    ...
    QUIET.hideChanges();
    const qrec = (pi) => ({ users: { admin: { password: 'pw!' }, '1': { pi } }, lastSave: { build: 14 } });
    ...
    const rec = (pi, build = 14) => ({ users: { admin: { password: 'pw!' }, '1': { pi } }, lastSave: { build } });
    ...
    r = await call('PUT', '', { baseRev: 5, record: rec(1, 13) }, D1ENV);
    check('D1: 예전 빌드는 426 으로 거부', r.status === 426 && r.body.needBuild === 14);
    check('D1: 그때도 저장소 그대로', JSON.stringify(D1.state) === beforeBuild);

    r = await call('PUT', '', { baseRev: 5, record: { note: '학생 정보 없음', lastSave: { build: 14 } } }, D1ENV);
